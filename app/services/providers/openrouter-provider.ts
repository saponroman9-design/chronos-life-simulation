...
export class OpenRouterProvider implements AIProvider {
  ...
  async generateText(
    ...
    try {
      const model = options?.model || process.env.OPENROUTER_TEXT_MODEL || "deepseek/deepseek-chat"
      ...
      const response = await fetch(`${this.config.baseUrl}/chat/completions`, {
        ...
        headers: {
          ...
          "HTTP-Referer": process.env.NEXT_PUBLIC_SITE_URL || "https://chronos-v6.vercel.app",
        },
        body: JSON.stringify({
          model,
          messages,
  ...
  async generateImage(_prompt: string, _options?: AIOptions): Promise<AIImageResult> {
    return {
      success: false,
      error: "Image generation through OpenRouter not yet implemented",
    }
  }
  ...
  async generateAudio(_text: string, _options?: AIOptions): Promise<AIAudioResult> {
    return {
      success: false,
      error: "Audio generation not supported by OpenRouter",
    }
  }
